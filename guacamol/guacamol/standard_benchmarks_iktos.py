...
from guacamol.guacamol.common_scoring_functions import (
    TanimotoScoringFunction,
    RdkitScoringFunction,
    CNS_MPO_ScoringFunction,
    IsomerScoringFunction,
    SMARTSScoringFunction,
    MaxTanimotoScoringFunction,
)
from guacamol.guacamol.common_scoring_functions_iktos import (
    RScoreScoringFunction,
    MaxTanimotoScoringFunction,
    QEDScoringFunction,
    mtorScoringFunction,
    pi3kScoringFunction,
...
from guacamol.guacamol.scoring_function import (
    ...
    ScoringFunction,
    BatchGeometricMeanScoringFunction,
)
from guacamol.guacamol.utils.descriptors import (
    num_aromatic_rings,
    logP,
    qed,
    tpsa,
...
def add_synth_scorer(list_scoring, name, mean_cls, score_synth, path_weights=None):
    if score_synth is not None:
        if score_synth == "RScore":
            ...
            os.environ[
                "SPAYA_API_URL"
            ] = "https://dev.retrosynthesis-api.maheo.tech/api/batch_smiles"
            os.environ["SPAYA_API_TOKEN"] = "KYifdgUw&nbBlGBqhosLvq0YnL"

            if not os.getenv("SPAYA_API_TOKEN"):
                raise Exception(
                    "No token for SpayaAPI was set in environement "
                    "variable SPAYA_API_TOKEN"
                )
            if not os.getenv("SPAYA_API_URL"):
                raise Exception("No  SPAYA_API_URL was set in environement variable")
...
def pi3kmtor_bench(score_synth) -> GoalDirectedBenchmark:
    ...
    similarity = MaxTanimotoScoringFunction(
        smiles_ref=smiles_ref,
        fp_type="ECFP4",
        score_modifier=MaxGaussianModifier(mu=0.75, sigma=0.25),
    )  # before (0.5, 0.1)
...
def similarity_and_synth(
    ...
    tanimoto = TanimotoScoringFunction(
...
def median_camphor_menthol_and_synth(
    score_synth, mean_cls=GeometricMeanScoringFunction
) -> GoalDirectedBenchmark:
...
def hard_osimertinib_and_synth(
    score_synth, mean_cls=GeometricMeanScoringFunction
) -> GoalDirectedBenchmark:
    ...
    list_scoring = [
        similar_to_osimertinib,
        but_not_too_similar,
        tpsa_over_100,
        logP_scoring,
    ]
    scoring_function_start = mean_cls(list_scoring)
    make_osimertinib_great_again, name = add_synth_scorer(
        list_scoring, name, mean_cls, score_synth
    )
...
def hard_fexofenadine_and_synth(
    score_synth, mean_cls=GeometricMeanScoringFunction
) -> GoalDirectedBenchmark:
    """
    ...
    optimize_fexofenadine, name = add_synth_scorer(
        list_scoring, name, mean_cls, score_synth
    )
    specification = uniform_specification(1, 10, 100)
...
def ranolazine_mpo_and_synth(
    ...
    optimize_ranolazine, name = add_synth_scorer(
        list_scoring, name, mean_cls, score_synth
    )

    scoring_function_start = GeometricMeanScoringFunction(
        [similar_to_ranolazine, logP_under_4, tpsa_f, fluorine]
    )
...
def perindopril_rings_and_synth(score_synth,) -> GoalDirectedBenchmark:
    ...
    perindopril = TanimotoScoringFunction(smiles, fp_type="ECFP4")
...
def smarts_with_other_target_and_synth(
    smarts: str, other_molecule: str, score_synth
) -> ScoringFunction:
...
def isomers_c7h8n2o2_and_synth(
    mean_function="geometric", score_synth=None
) -> GoalDirectedBenchmark:
    """
    ...
    objective, name = add_synth_scorer(
        [objective_start],
        "isomers_c7h8n2o2",
        mean_cls=mean_cls,
...
def valsartan_smarts_and_synth(
    score_synth, mean_cls=GeometricMeanScoringFunction
) -> GoalDirectedBenchmark:
    ...
    specification = uniform_specification(1, 10, 100)
    objective, scoring_function_start, name = smarts_with_other_target_and_synth(
        valsartan_smarts, sitagliptin_smiles, score_synth
    )
...
def decoration_hop_and_synth(score_synth) -> GoalDirectedBenchmark:
    ...
    scoring_function_start = ArithmeticMeanScoringFunction(
...
def logP_benchmark_iktos(target: float, score_synth=None) -> GoalDirectedBenchmark:
    ...
    scoring, benchmark_name = add_synth_scorer(
        [scoring], benchmark_name, GeometricMeanScoringFunction, score_synth
    )
    specification = uniform_specification(1, 10, 100)
...
def tpsa_benchmark_iktos(target: float, score_synth) -> GoalDirectedBenchmark:
    ...
    tpsa_scoring = RdkitScoringFunction(
        descriptor=tpsa, score_modifier=GaussianModifier(mu=target, sigma=20.0)
    )
    scoring = tpsa_scoring
    specification = uniform_specification(1, 10, 100)
    mean_cls = GeometricMeanScoringFunction
    scoring_function, benchmark_name = add_synth_scorer(
        [scoring], benchmark_name, mean_cls, score_synth
    )
    return GoalDirectedBenchmark(
        name=benchmark_name,
        objective=scoring_function,
...
def cns_mpo_iktos(score_synth, max_logP=5.0) -> GoalDirectedBenchmark:
    ...
    mean_cls = GeometricMeanScoringFunction
    scoring_function, benchmark_name = add_synth_scorer(
        [scoring], name, mean_cls, score_synth
    )
    return GoalDirectedBenchmark(
        name=benchmark_name,
        objective=scoring_function,
