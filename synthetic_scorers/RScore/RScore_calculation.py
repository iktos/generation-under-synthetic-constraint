...
def interact_with_api(smiles_list, timeout=1):
    url = os.getenv(
        "SPAYA_API_URL")  #"https://dev.retrosynthesis-api.maheo.tech/api/batch_smiles"
    token = os.getenv("SPAYA_API_TOKEN")  # "KYifdgUw&nbBlGBqhosLvq0YnL"
...
def get_RScore(smiles_list) -> float:
    ...
    try:
        while len(smiles_list) > 0 and time.perf_counter() - t1 < max_time:
            ...
            smiles_list = [s for s, val in results.items() if
                           val["status"] not in ["DONE", "INVALID SMILES"]]
    ...
    print("time to score  {} molecules in min : {}".format(n_smiles, int(
        (time.perf_counter() - t1) / 60)))
