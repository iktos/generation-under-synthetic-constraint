...
class PPODirectedGenerator(GoalDirectedGenerator):
    def __init__(self, pretrained_model_path: str, num_epochs=4, episode_size=1028, batch_size=512,
                 entropy_weight=1, kl_div_weight=5, clip_param=0.2, number_final_samples=1028,
                 sample_final_model_only=False, database_name=None) -> None:
    ...
    def generate_optimized_molecules(self, scoring_function: ScoringFunction, number_molecules: int,
                                     starting_population: Optional[List[str]] = None, scoring_function_start=None,
                                    benchmark_name=None) -> List[str]:
        if scoring_function_start==None:
            scoring_function_start=scoring_function
        ...
        generator = PPOMoleculeGenerator(model=model,
                                         ...
                                         device=device, database_name = self.database_name)
